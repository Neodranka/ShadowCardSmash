卡牌数据需要替换成设计文档中的版本。请删除 VampireCards.cs 中的现有卡牌，替换为以下设计：

## 替换 VampireCards.cs 的内容
```csharp
    ...
    public static class VampireCards
    {
            ...
            return new CardData
            {
                ...
                effects = new List<EffectData>
                {
                    new EffectData
                    {
                        ...
                        targetType = TargetType.PlayerChoice,
                        parameters = new List<string> { "2", "1" }
                    },
                    new EffectData
                    {
                        trigger = EffectTrigger.OnPlay,
            ...
            return new CardData
            {
                ...
                evolveEffects = new List<EffectData>
                {
                    new EffectData
                    {
                        ...
                        effectType = EffectType.Buff,
                        targetType = TargetType.Self,
                        parameters = new List<string> { "-2", "-2" }
                    },
                    new EffectData
                    {
                        trigger = EffectTrigger.OnEvolve,
            ...
            return new CardData
            {
                ...
                effects = new List<EffectData>
                {
                    ...
                    new EffectData
                    {
                        ...
                        effectType = EffectType.Buff,
                        targetType = TargetType.Self,
                        parameters = new List<string> { "-30", "-30" }
                    },
                    // 谢幕：对方回合时触发
                    new EffectData
                    {
                ...
                enhanceEffects = new List<EffectData>
                {
                    ...
                    new EffectData
                    {
                        ...
                        effectType = EffectType.Buff,
                        targetType = TargetType.Self,
                        parameters = new List<string> { "-27", "-27" },
        ...
        private static CardData CreateLongAwaitedHunt()
        {
            return new CardData
            {
                ...
                description = "对所有敌方随从与敌方玩家造成6点伤害，根据被破坏的随从数使我方场上的"沉睡的饥渴 利维耶·西提恩茨"获得+1/+1。",
                isToken = true,
                effects = new List<EffectData>
                {
                    new EffectData
...
// TargetType 枚举中添加（如果没有）
All,              // 所有随从+双方玩家
SingleMinion,     // 单个随从（双方都可选）
AllEnemies,       // 所有敌方（随从+玩家）
```

2. 在 CardData.cs 添加：
```csharp
public bool canEvolveWithEP = true;  // 是否可以用EP进化
public bool isToken;                  // 是否是TOKEN卡
public List<EffectData> evolveEffects;  // 进化时效果
```

3. 在 EffectData.cs 添加：
```csharp
public bool overrideNormalEffect;  // 强化时是否替代普通效果
```
